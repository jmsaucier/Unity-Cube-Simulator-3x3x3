...
public class CubeController : MonoBehaviour
{
    ...
    private void initTrail()
    {
        ...
        {
            foreach (TrailRenderer trailRenderer in trailRends)
            {
                trailRenderer.enabled = GameObject.Find("trail_toggle").GetComponent<Toggle>().isOn;
            }
        }
    ...
    private void Rotation()
    {
        ...
        if (rotate)
        {
            if (!rotateInit)
            {
                ...
                if (reverse)
                { // a little cheat so I don't have to code too much = slight performance impact :]
                    cube2d.Rotate(selection);
                    cube2d.Rotate(selection);
                }
